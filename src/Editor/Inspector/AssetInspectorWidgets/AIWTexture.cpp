...
Texture2D *AIWTexture::GetTexture() const
{
    return SCAST<Texture2D *>(GetAsset().Get());
}

void AIWTexture::UpdateInputsFromAsset()
{
...
Texture2D *AIWTexture::GetIconTexture() const
{
    return GetTexture();
}

void AIWTexture::OnValueChangedAIWAsset(EventEmitter<IEventsValueChanged> *)
{
    Path texImportPath =
        MetaFilesManager::GetMetaFilepath(GetTexture()->GetAssetFilepath());
